...
function nowLocation(){
    AMap.plugin(['AMap.Geolocation'], function() {
        var geolocation = new AMap.Geolocation({
            ...
            buttonOffset: new AMap.Pixel(14, 200),//定位按钮与设置的停靠位置的偏移量，默认：Pixel(10, 20)
...
function drivingPointPoute(lng,lat){
    ...
    driving.search(
        ...
        new AMap.LngLat(121.506436,31.099885), //当前位置
...
$('.compass').click(function () { 
    ...
    if(lineArr.length == 0){
        return
    }else if(lineArr.length > 0 && compassClick){
        startAnimation()
        compassClick = false
    }else if(lineArr.length > 0 && compassClick == false){
        resumeAnimation()
        // compassClick = true
    }
});

$('.rear-view').click(function () { 
    pauseAnimation()
});